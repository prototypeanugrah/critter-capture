...
echo "[inference] Initializing ZenML..."
uv run zenml init

echo "[inference] Authenticating with local ZenML server..."
if ! uv run zenml logout --local >/dev/null 2>&1; then
  echo "[inference] No existing session to log out from."
fi
uv run zenml login --local

echo "[inference] Ensuring MLflow experiment tracker exists..."
...
echo "[inference] Ensuring MLflow model deployer exists..."
...
echo "[inference] Configuring local_mlflow_stack..."
...
echo "[inference] Running inference pipeline..."
